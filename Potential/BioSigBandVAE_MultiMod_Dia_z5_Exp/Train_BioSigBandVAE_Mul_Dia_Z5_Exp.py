...
if __name__ == "__main__":
    ...
    '''
    ReconOut_ext = MSE(ReconOut_ext, EncInp)
    ReconOut_gen = MSE(ReconOut_gen, EncInp)
    RecLoss = Beta_Rec * tf.reduce_max([ReconOut_ext, ReconOut_gen])
    ...
    RelLossDic = {'val_ReconOut_gen':'Beta_Rec_gen', 'val_ReconOut_ext':'Beta_Rec_ext', 'val_FeatRecLoss':'Beta_Feat', 'val_kl_Loss_Z':'Beta_Z', 'val_kl_Loss_FC':'Beta_Fc'}
    ScalingDic = {'val_ReconOut_gen':100., 'val_ReconOut_ext':100., 'val_FeatRecLoss':100., 'val_kl_Loss_Z':0.1, 'val_kl_Loss_FC':0.1}
    MinLimit = {'Beta_Rec_gen':1., 'Beta_Rec_ext':1., 'Beta_Feat':1., 'Beta_Z':0.01, 'Beta_Fc':0.01}
    MaxLimit = {'Beta_Rec_gen':500.,'Beta_Rec_ext':500., 'Beta_Feat':500., 'Beta_Z':0.17, 'Beta_Fc':0.17}
    ...
    SigBandRepModel.fit(TrData, batch_size=3000, epochs=1500, shuffle=True, validation_data =(ValData, None) , callbacks=[  RelLoss]) 
