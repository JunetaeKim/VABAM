...
def FACLosses (Models, LossConfigSet):
    ...
    ## Model component splits
    EncInp_D1, EncInp_D2 = SplitBatch(EncModel.input, HalfBatchIdx1, HalfBatchIdx2, mode='Both')
    Z_D1, Z_D2 = SplitBatch(EncModel.output[2], HalfBatchIdx1, HalfBatchIdx2, mode='Both')
    InpFCCommon_D1, InpFCCommon_D2 = SplitBatch(EncModel.output[1][:, :2], HalfBatchIdx1, HalfBatchIdx2, mode='Both')
    InpFCEach_D1, InpFCEach_D2 = SplitBatch( EncModel.output[1][:, 2:], HalfBatchIdx1, HalfBatchIdx2, mode='Both')

    SigOut , FCs = EncModel.output[:2]
    SigOut_D1, SigOut_D2 = SplitBatch( SigOut, HalfBatchIdx1, HalfBatchIdx2, mode='Both')
    FCs_D1, FCs_D2 = SplitBatch( FCs, HalfBatchIdx1, HalfBatchIdx2, mode='Both')

    ## Each output of each model
    FeatExtOut_D1 = FeatExtModel([SigOut_D1, FCs_D1])
    FeatGenOut_D1 = FeatGenModel([InpFCCommon_D1, InpFCEach_D1, Z_D1])
    ReconExtOut_D1 = ReconModel(FeatExtOut_D1)
    ReconGenOut_D1 = ReconModel(FeatGenOut_D1)
    FacDiscOut_D1 =  FacDiscModel(Z_D1)

    ### Define the total model
    SigRepModel = Model(EncInp_D1, [FacDiscOut_D1, FeatGenOut_D1, ReconExtOut_D1, ReconGenOut_D1])
    ...
    ### KL Divergence for p(Z) vs q(Z)
    Z_Mu_D1, Z_Log_Sigma_D1 = SigRepModel.get_layer('Z_Mu').output, SigRepModel.get_layer('Z_Log_Sigma').output
    ...
    PermZ_D2 = tf.concat([tf.nn.embedding_lookup(Z_D2[:, i][:,None], tf.random.shuffle(tf.range(Batch2_Size))) for i in range(LatDim)], axis=1)
    ...
    ### KL Divergence for p(FC) vs q(FC)
    BernP = 0.5 # hyperparameter
    FC_Mu_D1 = SigRepModel.get_layer('FC_Mu').output 
