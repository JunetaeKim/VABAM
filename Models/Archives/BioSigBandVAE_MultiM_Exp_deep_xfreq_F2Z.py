...
def Encoder(SigDim, LatDim= 2, Type = '', MaskingRate = 0.025, NoiseStd = 0.002, MaskStd = 0.1, ReparaStd = 0.1 , Reparam = False, FcLimit=0.1):
    ...
    FC_Mu =   Dense(6, activation='sigmoid')(Encoder)
    ...
    ToZs = Dense(LatDim, activation='relu')(FCs)
    ToZs = Dense(LatDim, activation='relu')(ToZs)
    ToZs = Dense(LatDim, activation='relu')(ToZs)
    
    Z_Mu = Dense(LatDim, activation='linear', name='Z_Mu')(ToZs)
    Z_Log_Sigma = Dense(LatDim, activation='softplus')(ToZs)
