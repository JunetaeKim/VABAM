...
def Encoder(SigDim, LatDim= 2, Type = '', MaskingRate = 0.025, NoiseStd = 0.002, MaskStd = 0.1, ReparaStd = 0.1 , Reparam = False, FcLimit=0.1):
    ...
    Z_Mu = Dense(LatDim*4, activation='linear', name='Z_Mu')(Encoder)
    Z_Log_Sigma = Dense(LatDim*4, activation='softplus')(Encoder)
...
## --------------------------------------------------   FeatExtractor  -------------------------------------------------------------
def FeatExtractor(SigDim, FiltLenList = [301, 301, 301, 301, 301, 301], DecayH = 0. , DecayL = 0. ):
...
def FeatGenerator (LatDim= 2):
    ...
    InpZ = Input(shape=(LatDim*4,), name='Inp_Z')
    FCCommon = Input(shape=(2,), name='Inp_FCCommon')
    FCEach = Input(shape=(4,), name='Inp_FCEach')
    
    HH_F, HL_F, LH_F, LL_F = tf.split(FCEach, 4, axis=1)
    HH_Z, HL_Z, LH_Z, LL_Z = tf.split(InpZ, 4, axis=1)
    
   
    Dec_Sig_HH = Dense(20, activation='relu')(tf.concat([HH_Z, FCCommon, HH_F], axis=-1))
    ...
    Dec_Sig_HL = Dense(20, activation='relu')(tf.concat([HL_Z, FCCommon, HL_F], axis=-1))
    ...
    Dec_Sig_LH = Dense(20, activation='relu')(tf.concat([LH_Z, FCCommon, LH_F], axis=-1))
    ...
    Dec_Sig_LL = Dense(20, activation='relu')(tf.concat([LL_Z, FCCommon, LL_F], axis=-1))
