...
if __name__ == "__main__":
    ...
    #### -----------------------------------------------------   Experiment setting   -------------------------------------------------------------------------    
    SavePath = './Results/'
    ModelName = 'SigBandRep_MultMod_Dia_Z3.hdf5'
    ...
    ReconOut_ext = Beta_Rec * MSE(ReconOut_ext, EncInp)
    SigBandRepModel.add_loss(ReconOut_ext)
    SigBandRepModel.add_metric(ReconOut_ext, 'ReconOut_ext')
    
    ReconOut_gen = Beta_Rec * MSE(ReconOut_gen, EncInp)
    ...
    FeatLossWei = tf.concat([ tf.abs(tf.reduce_max(i, keepdims=True )) for i in FeatExtOut], axis=0)
    ...
    FeatRecLoss= Beta_Feat * MSE(tf.concat(WeigFeatGenOut, axis=-1), tf.concat(WeigFeatExtOut, axis=-1))
    ...
    RelLossDic = {'val_ReconOut_gen':'Beta_Rec', 'val_FeatRecLoss':'Beta_Feat', 'val_kl_Loss_Z':'Beta_Z', 'val_kl_Loss_FC':'Beta_Fc'}
    ScalingDic = {'val_ReconOut_gen':100., 'val_FeatRecLoss':100., 'val_kl_Loss_Z':0.1, 'val_kl_Loss_FC':0.1}
    MinLimit = {'Beta_Rec':1., 'Beta_Feat':1., 'Beta_Z':0.01, 'Beta_Fc':0.01}
    MaxLimit = {'Beta_Rec':500., 'Beta_Feat':500., 'Beta_Z':0.04, 'Beta_Fc':0.08}
    RelLoss = RelLossWeight(BetaList=RelLossDic, LossScaling= ScalingDic, MinLimit= MinLimit, MaxLimit = MaxLimit, ToSaveLoss=['val_FeatRecLoss', 'val_ReconOut_gen'] , SaveWay='max' , SavePath = ModelSaveName)
    ...
    SigBandRepModel.fit(TrData, batch_size=3500, epochs=900, shuffle=True, validation_data =(ValData, None) , callbacks=[  RelLoss]) 
