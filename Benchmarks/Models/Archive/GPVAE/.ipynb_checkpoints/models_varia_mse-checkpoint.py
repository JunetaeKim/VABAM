...
class JointEncoderGRU(tf.keras.Model):
    def __init__(self, z_size, hidden_sizes=(64, 64), transpose=False, **kwargs):
        ...
        """
        super(JointEncoderGRU, self).__init__()
        self.z_size = int(z_size)
        self.net = make_GRU_Encoder(2*z_size, hidden_sizes)
...
class BandedJointEncoderGRU(tf.keras.Model):
    def __init__(self, z_size, hidden_sizes=(64, 64), data_type=None, transpose=False, **kwargs):
        ...
        """
        super(BandedJointEncoderGRU, self).__init__()
        self.z_size = int(z_size)
        self.net = make_GRU_Encoder(3*z_size, hidden_sizes)
...
class Decoder(tf.keras.Model):
    def __init__(self, output_size, hidden_sizes=(64, 64)):
        ...
        self.net = make_GRU_Decoder(output_size, hidden_sizes)
...
class VAE(tf.keras.Model):
    ...
    def _compute_loss(self, x, m_mask=None, return_parts=False):
        ...
        mse = self.compute_mse(x) # mse instead of nll 
        ...
        if self.K > 1:
            ...
            kl = tf.reduce_sum(kl, 1)  # shape=(M*K*BS)
            kl = tf.reduce_mean(self.beta * kl)  # scalar
